...
KubeDB **v2026.2.x** focuses on **operational efficiency, GitOps safety, high-availability improvements, and advanced recovery capabilities** across the entire database ecosystem. This release enhances OpsRequest execution speed, improves TLS and configuration automation, introduces PostgreSQL read replicas and migration tooling, expands monitoring support, and strengthens distributed database capabilities.
...
* PostgreSQL Migration Tool (Live + CDC-based)
...
* New PostgreSQL versions: **18.2, 17.8, 16.12, 15.16, 14.21**
...
- Failover still works normally using your small number of full-strength standbys.
...
Let us know if you'd like examples of how to configure this in the CRD!
...
you can also set below fileds in the `.spec.readReplicas`
...
# ────────────────────────────────────────────────
  ...
  # ────────────────────────────────────────────────
  # 8. PodPlacementPolicy (reference to a PodPlacementPolicy CR)
  podPlacementPolicy:
    name: postgres-preferred          # must exist in same namespace
...
New Postgres WAL-Log Retention Feature
...
retentionPeriod: Duration after which old WAL files are deleted (e.g., 10d)
